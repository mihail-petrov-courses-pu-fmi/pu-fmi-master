...
public class ApplicationLoader {
    ...
    public Object getInjectable(Class clazzKey, boolean asSingleton) {

        if(asSingleton) {
            return this.getInjectable(clazzKey);
        }

        if(this.injectableLookupTable.containsKey(clazzKey)) {
            return this.getInjectableInstance(clazzKey);
        }

        return null;
    }

    // As Singleton by default
    public Object getInjectable(Class clazzKey) {

        Object resultInstance =  this.injectableLookupTable.get(clazzKey);

        if(resultInstance == null) {
            resultInstance =  this.getInjectableInstance(clazzKey);
            this.injectableLookupTable.put(clazzKey, resultInstance);
        }

        return resultInstance;
    }

    private Object getInjectableInstance(Class clazzKey) {

        try {
            return clazzKey.getDeclaredConstructor().newInstance();
   ...
   private void classParser(String packageReference, String packageName) throws ClassNotFoundException {
       ...
       if(clazz.isAnnotationPresent(org.example.steriotypes.Injectable.class)) {
        this.parseInjectable(clazz);
       }
   }
